...
let inventory: InventoryItem[] = JSON.parse(localStorage.getItem('inventoryData') || '[]');
...
/** 验证输入合法性 */
function validateInputs(id: string, name: string, qty: number, price: number, supplier: string): boolean {
...
function showMessage(msg: string): void {
    ...
    // 3秒后自动清空提示
    setTimeout(() => {
        if (outputEl.innerHTML.includes(msg)) outputEl.innerHTML = "";
    }, 3000);
}
...
function addOrUpdateItem(): void {
    ...
    const popularItemEl = document.getElementById("popularItem") as HTMLSelectElement;
    const commentEl = document.getElementById("comment") as HTMLTextAreaElement;

    // 检查元素是否存在
    const missingElements: string[] = [];
    if (!itemIdEl) missingElements.push("Item ID");
    if (!itemNameEl) missingElements.push("Item Name");
    if (!categoryEl) missingElements.push("Category");
    if (!quantityEl) missingElements.push("Quantity");
    if (!priceEl) missingElements.push("Price");
    if (!supplierEl) missingElements.push("Supplier");
    if (!stockStatusEl) missingElements.push("Stock Status");
    if (!popularItemEl) missingElements.push("Popular Item");
    if (!commentEl) missingElements.push("Comment");
    ...
    // 验证输入
    if (!validateInputs(itemId, itemName, quantity, price, supplier)) return;
...
function searchItems(): void {
    ...
    const outputEl = document.getElementById("output");
    if (!searchInputEl || !outputEl) return;
...
function deleteItemPrompt(): void {
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // 绑定按钮事件
    const saveBtn = document.querySelector('button[onclick="addOrUpdateItem()"]') as HTMLButtonElement;
    const showAllBtn = document.querySelector('button[onclick="showAllItems()"]') as HTMLButtonElement;
    const showPopularBtn = document.querySelector('button[onclick="showPopularItems()"]') as HTMLButtonElement;
    const deleteBtn = document.querySelector('button[onclick="deleteItemPrompt()"]') as HTMLButtonElement;
    const searchInput = document.getElementById("searchInput") as HTMLInputElement;

    // 安全绑定事件
    if (saveBtn) saveBtn.onclick = addOrUpdateItem;
    if (showAllBtn) showAllBtn.onclick = showAllItems;
    if (showPopularBtn) showPopularBtn.onclick = showPopularItems;
    if (deleteBtn) deleteBtn.onclick = deleteItemPrompt;
    if (searchInput) searchInput.oninput = searchItems;
});